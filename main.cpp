...
//            // Create a recursive_directory_iterator pointing to the end
//            boost::filesystem::recursive_directory_iterator end_iter;
//            // Loop through all files in the source directory, including ones in nested directories
//            for (boost::filesystem::recursive_directory_iterator dir_itr(src_dir); dir_itr != end_iter; ++dir_itr) {
//                // Check whether the current file is a regular file and whether its extension is .mp3
//                if (boost::filesystem::is_regular_file(dir_itr->status()) &&
//                    dir_itr->path().extension() == ".mp3") {
//                    // Construct the destination path
//                    boost::filesystem::path dest_path = dest_dir / dir_itr->path().filename();
//                    // Copy the file to the destination directory, overwriting a previous file with the same name if one exists
//                    boost::filesystem::copy_file(dir_itr->path(), dest_path, boost::filesystem::copy_option::overwrite_if_exists);
//                    // Print out which file was copied to where
//                    std::cout << "Copied " << dir_itr->path().string() << " to " << dest_path.string() << std::endl;
//                }
//            }
//        } else {
//            // Inform the user if the source path does not exist or is not a directory
//            std::cout << src_dir << " does not exist or is not a directory." << std::endl;
//        }
//    } catch (const boost::filesystem::filesystem_error& ex) {
//        // Catch and print any filesystem errors
